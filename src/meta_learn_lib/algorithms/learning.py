...
def rtrl_like[H, X, Y, Z, W](
    machine: Mealy[H, H, X, X, Y, Y, Z, Z],
    update_influence: Callable[[Callable[[jax.Array, jax.Array], jax.Array], Callable[[jax.Array], jax.Array], W], W],
    boundary: Callable[[H, Y, jax.Array, W], jax.Array],
) -> Mealy[tuple[H, W], tuple[H, W], X, X, Y, Y, Z, Z]:

    def run(
        p_sx: tuple[Z, tuple[tuple[H, W], X]],
    ) -> tuple[
        tuple[tuple[H, W], Y],
        Callable[[tuple[tuple[H, W], Y]], tuple[Z, tuple[tuple[H, W], X]]],
    ]:
        z, ((h0, W0), x) = p_sx
...
def rtrl[H, X, Y, Z](
    machine: Mealy[H, H, X, X, Y, Y, Z, Z],
) -> Mealy[tuple[H, JACOBIAN], tuple[H, JACOBIAN], X, X, Y, Y, Z, Z]:
    ...
    def boundary(d_h_final: H, d_y: Y, d_h0: jax.Array, M0: JACOBIAN) -> jax.Array:
        return d_h0 @ M0
...
def uoro[H, X, Y, Z](
...
) -> Mealy[tuple[H, UORO_AUX], tuple[H, UORO_AUX], X, X, Y, Y, Z, Z]:
    ...
    def boundary(d_h_final: H, d_y: Y, d_h0: jax.Array, W0: UORO_AUX) -> jax.Array:
        A0, B0, _ = W0
        return (d_h0 @ A0) * B0
