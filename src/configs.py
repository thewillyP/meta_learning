...
VAE_BASELINE = GodConfig(
    ...
    readout_graph={
        ...
        "concat": {"x"},
        "encoder_hidden1": {"concat"},
        "encoder_norm1": {"encoder_hidden1"},
        "encoder_out": {"encoder_norm1"},
        "latent": {"encoder_out"},
        "latent_z": {"latent"},
        "decoder_hidden1": {"latent_z"},
        "decoder_norm1": {"decoder_hidden1"},
        "decoder_out": {"decoder_norm1"},
        "decoder_reshape": {"decoder_out"},
        "merge": {"decoder_reshape", "latent"},
    },
    nodes={
        "x": UnlabeledSource(),
        "concat": Concat(),
        "encoder_hidden1": NNLayer(n=32, activation_fn="relu", use_bias=True),
        "encoder_norm1": GroupNorm(groups=1, epsilon=1e-5, channelwise_affine=True),
        "encoder_out": NNLayer(n=2 * 2, activation_fn="identity", use_bias=True),
        "latent": ReparameterizeLayer(),
        "latent_z": ExtractZ(n=2),
        "decoder_hidden1": NNLayer(n=32, activation_fn="relu", use_bias=True),
        "decoder_norm1": GroupNorm(groups=1, epsilon=1e-5, channelwise_affine=True),
        "decoder_out": NNLayer(n=784, activation_fn="identity", use_bias=True),
        "decoder_reshape": Reshape(shape=(1, 28, 28)),
        "merge": MergeOutputs(),
    },
    ...
    sample_generators=[
        SampleGeneratorConfig(
            ...
            readout_graph={
                ...
                "decoder_norm1": frozenset({"decoder_hidden1"}),
                "decoder_out": frozenset({"decoder_norm1"}),
